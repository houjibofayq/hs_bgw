...
static inline void ble_operation_perform(ble_operation_t *operation)
{
    ...
    case BLE_OPERATION_TYPE_CONNECT:
		...
		ESP_LOGI(TAG,"-%d- BLE gattc open device :%s",__LINE__,operation->device->name);
        break;
...
static void ble_operation_dequeue(ble_operation_t **queue)
{
    ...
    *queue = operation->next;
    ESP_LOGD(TAG, "-%d-Dequeue: type: %d, device: " MAC_FMT ", char: " UUID_FMT ", "
        "len: %u, val: %p",__LINE__, operation->type, MAC_PARAM(operation->device->mac),
...
{
    ESP_LOGI(TAG, "-%d- Starting BLE scan",__LINE__);
    if (scan_requested)
        return 0;
...
int ble_services_scan(mac_addr_t mac)
{
    ...
    xSemaphoreGiveRecursive(devices_list_semaphore);
	ESP_LOGI(TAG,"-%d- BLE gattc search_service :%s",__LINE__,dev->name);
    return ret;
}
...
static void gap_cb(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    case ESP_GAP_BLE_SCAN_START_COMPLETE_EVT:
		...
		ESP_LOGD(TAG,"-%d-Ble scan start conplete!",__LINE__);
        break;
    ...
    case ESP_GAP_BLE_SCAN_RESULT_EVT:
    {
		...
		if(!name)
		  break;
		ESP_LOGI(TAG,"-%d- Ble scan device name:%s",__LINE__,name);
		if(!strstr(name,"ISE03"))
		  break;
        ...
        ESP_LOGI(TAG,"-%d- Ble add devices_list, name:%s",__LINE__,name);
        xSemaphoreGiveRecursive(devices_list_semaphore);
    ...
    esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_SEARCH_CMPL_EVT:
        ...
        ESP_LOGI(TAG,"-%d- Ble gattc search complete",__LINE__);
        ...
        if (param->read.status != ESP_GATT_OK)
        {
                ...
                param->read.status == ESP_GATT_INSUF_ENCRYPTION)
            {
                if (!device->is_authenticating)
                {
                    ...
                    //esp_ble_set_encryption(device->mac,ESP_BLE_SEC_ENCRYPT_MITM);
                }
                /* Try again */
...
int ble_initialize(void)
{
    ...
    if (!(purge_device_list_timer = xTimerCreate("purge_device_list_timer",
        pdMS_TO_TICKS(60 * 60 * 1000), pdTRUE, NULL,       //one hour
