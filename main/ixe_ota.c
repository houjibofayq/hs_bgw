...
static void ixe_do_ota_task(void *pvParameter)
{
    ...
    if(strstr(x_ble.ble_name,"ISE03"))
	{
	  ...
	  strcpy(ota_url,"https://ota.lazelaze.com/firmware2/ISE03/ISE03_OTA.bin?sn=");
	  type = PRO_ISE03;
    }
	else if(strstr(x_ble.ble_name,"IPE01"))
	{
	  ...
	  ESP_LOGI(TAG,"[%d]BLE do not have correct name!",__LINE__);
...
//#endif
	...
	ESP_LOGI(TAG,"start get ota data [%d]\n",__LINE__);
    while (1) {
...
void ixe_ota_rallback()
{
  ...
  ESP_LOGI(TAG,"[%d]configued_partition type %d subtype %d at offset 0x%08x,size 0x%08x\n",__LINE__,
				 configured->type, configured->subtype, configured->address,configured->size);
  ESP_LOGI(TAG,"[%d]running_partition type %d subtype %d at offset 0x%08x,size 0x%08x \n",__LINE__,
				 running->type, running->subtype, running->address, running->size);
  ESP_LOGI(TAG,"[%d]update_partition type %d subtype %d at offset 0x%08x,size 0x%08x \n",__LINE__,
				  update_partition->type,update_partition->subtype,update_partition->address,update_partition->size);
...
void ixe_ota_task(void *pvParameter)
{
    ...
    ESP_LOGI(TAG,"ixe start ota...[%s:%d]\n",__FILE__,__LINE__);
    ixe_ble_stop();
	sleep(2);
    ...
    ESP_LOGI(TAG,"...ota before check over... [%d:%s]\n",__LINE__,__FILE__);
    ...
    while(1)
    {
	  ...
	  {  
	     sleep(2);
	     ESP_LOGI(TAG,"[%s:%d] ESP  ixe_ota_task delete!\n",__FILE__,__LINE__);
	     vTaskDelete(NULL);
		 
	  }
	  if(ota_t++ >= 60*3)
	  {
        ESP_LOGI(TAG,"...Ota timout,restart the system!...\n");
		sleep(2);
		xota_set_status(UPDATE_FAIL);
		ESP_LOGI(TAG,"[%d] ESP  ixe_ota_task delete!\n",__LINE__);
	    vTaskDelete(NULL);
		ota_t = 0;
	  }
	}
